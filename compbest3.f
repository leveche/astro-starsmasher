...
c               write(69,*)x(i),y(i),z(i)
      ...
      if (ncomp1.eq.0 .and. ncomp2.eq.0 .and. ncomp3.eq.0) then
         ...
         if(.not. firstt) then
            if(myrank.eq.0)then
               ...
               write(69,*)'if stars are found, there is no guarantee'//
     $              ' that they will be the same component numbers as'//
     $              ' in the previous iteration'
      ...
      if(myrank.eq.0)write(69,'(a,7a10)') '       ','mass   ','x   ',
     $     'y   ','z   ','vx   ','vy   ','vz   '
...
c     that component to component 4, i.e. treat it like ejecta:
     ...
     $        ((x3-x12)**2+(y3-y12)**2+(z3-z12)**2)**0.5d0
         ...
         if(e1.ge.max(e2,e3) .and. e1.gt.0.d0)then
            ...
            stop 'Trouble finding ejected star. See log.'
         endif

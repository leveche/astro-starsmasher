...
c     main program loop:
     ...
     $                 'has overflowed an outer lagrangian point'
                  ...
                  stop 'Overflowed outer lagrangian point. See log.'
               endif
            enddo
         endif
...
c      end if
      ...
      updateduraddot=.false.

      call advance

      if(updateduraddot.and.myrank.eq.0) then
         write(699, *) t, cooling_update_frequency
      end if
...
*****************************************************************
      ...
      if(nprocs.gt.nprocsmax)then
         stop 'increase nprocsmax in starsmasher.h'
      endif
...
!     the following line assumes ppn cpu threads and abs(ngravprocs) gpu threads per node
      ...
      if(ngravprocs.gt.ngravprocsmax)then
         stop 'increase ngravprocsmax in starsmasher.h'
      endif
...
!      endif
      ...
      if (ierr .ne. MPI_SUCCESS) then
         stop 'Error getting processor name. Terminating.'
      end if
...
!     the following line assumes 8 cpu threads per node
 ...
 1235       if(irank.ne.ngravprocs-1) then
               stop 'problem assigning particles to grav processes'
            endif

...
c      endif
     ...
     $        gam
         stop 'Set gam=2 in sph.input, and try again. See log.'
      endif
...
c                  endif
     ...
     $              1.9d0*nnopt/anumden)**(1.d0/3.d0)
      ...
      if(ixmax.gt.hcpmax .or. iymax.gt.hcpmax .or.izmax.gt.hcpmax)then
         ...
         stop 'ixmax,iymax,izmax. See log.'
      endif
...
c               if(p.eq.298580 .or. p+1.eq.298580) then
     ...
     $              dpcartgrid(i+1,j,  k),
     $              dpcartgrid(i,  j,  k)
                  stop 'Last number is dhcpgrid. See log.'
               endif

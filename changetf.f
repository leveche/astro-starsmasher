...
c     $           rho, recvcounts,displs, mpi_double_precision, irank,
...
c         if(myrank.eq.0)write(69,*)'timelast being set to',timelast
     ...
     $     .or.red+green+blue.ne.6) then
         ...
         call flush(69)
         error stop 'Star not assigned properly. See log.'
      endif
...
c            stop
     ...
     $           deltaamb,deltaamg,deltaamr
            call flush(69)
            error stop 'Cannot determine absorber. See log.'
         endif
...
c     p=(gam-1)*rho*u=a*rho^gam, so u=a*rho^(gam-1)/(gam-1)
         ...
         if(abs(1.d0-eint/eintsave).gt.0.01d0) then
            if(myrank.eq.0)write(69,*)'u is changing a lot: will '//
     $           'integrate longer'
...
c               stara='r'
               istara=3
               error stop 'stara should not be red'
            else
c               starb='r'
               istarb=3
            endif
...
c     shift back to original inertial reference frame:
     ...
     $              actualspeed3rdinf,'  % t=',t
         ...
         if(myrank.eq.0)write(69,*) 'there are no binaries or other'//
     $        ' multiples'
         mejecta=am4
         ekinejecta=0.d0
         eintejecta=0.d0
     ...
     $     ekinejecta,eintejecta,epotejecta
      ...
      if(abs(am4-mce-mejecta).gt.1.d-5) then
         ...
         call flush(69)
         error stop 'Conservation of mass problem. See log.'
      endif

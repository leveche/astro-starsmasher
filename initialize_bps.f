...
c break.  and that doesn't seem to be happening fortunately!
     ...
     $     alphaold,betaold,trelaxold,dtold
      ...
      if(dummy.ne.'binary:')then
         stop 'input.bs should have binary first'
      endif
      read(30,*) am1chk
      ...
      if(dummy.ne.'single:')then
         stop 'input.bs should have single star second'
      endif
      read(30,*) am3chk
...
c         else if(xold.gt.0.d0) then
     ...
     $        dabs(deltavz2-vzcm2).gt.1.d-2) then
            write(69,*)'warning: changing v by a lot from '//
     $           'circular value'
            ...
            stop 'Changing v by a lot from circular value. See log.'
         endif
...
c      write(69,*) '...end natural units'
     ...
     $     dabs(zcm/zcombin-1.d0).gt.1.d-12)then
         ...
         stop 'COM position of binary is not correct. See log.'
      endif
     ...
     $     dabs(vzcm/vzcombin-1.d0).gt.1.d-11)then
         ...
         stop 'COM velocity of binary is not correct (2). See log.'
      endif
...
c     place velocities at same time as everything else:
     ...
     $        then
            stop "Mass(es) in sph.start?u doesn't match. See log."
         else

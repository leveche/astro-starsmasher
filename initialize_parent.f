...
c     do loop to get total gravitational potential energy:
     ...
     $              meanmolecular(i).gt.1.d-23) then
                  ...
                  stop 'Mean molecular problem. See log.'
               endif
            enddo
         endif
      endif
...
c     profilefile comes from MESA
 ...
 99      continue
         ...
         if(one.ne.1) then
            stop 'wait... I thought this was a MESA file...'
         endif
         read(io,*)             ! names of scalar variables associated with those integers                   
...
c     $           z_mass_fraction_metals, (dummy, ii=1,26),xm(i)
     ...
     $              z_mass_fraction_metals - 1d0
               stop 'X + Y + Z - 1 should equal 0. See log.'
            endif
...
c            xm(i)=star_mass*qq*1.9892d33     
     ...
     $              z_mass_fraction_metals -1d0,'mass problem in zone',k          
               stop 'Mass problem in zone. See log.'      
            endif
            i=i-1        
         enddo           
         close(io)       
...
c         write(101,*) i,uarray(i)
         ...
         if(myrank.eq.0)write(69,*) 'this initialization script '//
     $        'currently assumes that the eos is ideal gas + '//
     $        'radiation pressure (neos=1) or the eos is tabulated '//
     $        '(neos=2)'
         stop 'eos problem. See log.'
      endif
      if(myrank.eq.0)then

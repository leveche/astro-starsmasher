 ...
 8    if(myrank.eq.0)
     ...
     $     xtablelast,'in steps of',stepx
      if(numx.gt.maxnumx) then
         stop 'Increase maxnumx to accomodate EOS file data'
      endif
      goto 102
 98   if(myrank.eq.0)
...
c     write(69,*)'reading ',trim(eosfile)
     ...
     $           rhotable1-rhotable(1)
            stop 'rhotable(1) mismatch. See log.'
         endif
         if(utable1.ne.utable(1))then
            stop 'utable(1) mismatch'
         endif
         if(rhotablelast.ne.rhotable(numrho))then
            stop 'rhotable(numrho) mismatch'
         endif
         if(utablelast.ne.utable(numu))then
            stop 'utable(numu) mismatch'
         endif
...
c      endif
      ...
      if((ix.lt.1 .or. ix.ge.numx) .and. numx.gt.1) then
         ...
         stop 'Expand X range covered by EOS file. See log.'
      endif
